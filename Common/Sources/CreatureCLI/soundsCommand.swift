    ...
    struct Sounds: AsyncParsableCommand {
        ...
        struct List: AsyncParsableCommand {
            ...
            func run() async throws {
                try await tracedRun("sounds.list", config: globalOptions) {
                        ...
                        printTable(
                            ...
                            columns: [
                                ...
                                TableColumn(
                                    title: "File Size",
                                    valueProvider: {
                                        "\(formatNumber(UInt64($0.size))) bytes"
                                    }
                                ),
                                TableColumn(
                    ...
                    case .failure(let error):
                        throw failWithMessage(
                            "Error fetching the available sounds: \(ServerError.detailedMessage(from: error))"
                        )
                    }
                }
            }
        ...
        struct Play: AsyncParsableCommand {
            ...
            func run() async throws {
                ...
                try await tracedRun("sounds.play", config: globalOptions) {
                    let server = await Sounds.makeServer(for: globalOptions)
                    ...
                    case .failure(let error):
                        throw failWithMessage(
                            "Unable to play sound: \(ServerError.detailedMessage(from: error))")
                    }
                }
            }
        }
        ...
        struct GenerateLipSync: AsyncParsableCommand {
            ...
            func run() async throws {
                ...
                try await tracedRun("sounds.generate-lip-sync", config: globalOptions) {
                    let server = await Sounds.makeServer(for: globalOptions)
                    let result = await server.generateLipSync(
                        for: fileName, allowOverwrite: overwrite)
                    ...
                    case .failure(let error):
                        throw failWithMessage(
                            "Unable to generate lip sync: \(ServerError.detailedMessage(from: error))"
                        )
                    }
                }
            }
        ...
        struct GenerateLipSyncFromFile: AsyncParsableCommand {
            ...
            func run() async throws {
                ...
                guard !isDirectory.boolValue else {
                    throw failWithMessage(
                        "Input path \(inputURL.path) is a directory. Provide a WAV file.")
                }
                ...
                do {
                    ...
                    throw failWithMessage(
                        "Unable to read WAV file: \(error.localizedDescription)")
                }
                ...
                try await tracedRun("sounds.generate-lipsync-from-file", config: globalOptions) {
                    let server = await Sounds.makeServer(for: globalOptions)
                    ...
                    if let output {
                        ...
                        do {
                            ...
                            throw failWithMessage(
                                "Failed to write lip sync JSON: \(error.localizedDescription)")
                        }

                        print(
                            "Saved lip sync JSON to \(destinationURL.path) (\(response.data.count) bytes)"
                        )
                    } else {
        ...
        struct Download: AsyncParsableCommand {
            ...
            func run() async throws {
                try await tracedRun("sounds.download", config: globalOptions) {
        ...
        struct AdHoc: AsyncParsableCommand {
            ...
            struct List: AsyncParsableCommand {
                ...
                func run() async throws {
                    try await tracedRun("sounds.ad-hoc.list", config: globalOptions) {
                        let server = await Sounds.makeServer(for: globalOptions)
                        ...
                        case .failure(let error):
                            throw failWithMessage(
                                "Error fetching ad-hoc sounds: \(ServerError.detailedMessage(from: error))"
                            )
                        }
                    }
                }
            ...
            struct Download: AsyncParsableCommand {
                ...
                func run() async throws {
                    ...
                    try await tracedRun("sounds.ad-hoc.download", config: globalOptions) {
                        try await Sounds.performDownload(
...
extension CreatureCLI.Sounds {
    ...
    fileprivate static func performDownload(
        ...
        case .failure(let error):
            throw failWithMessage(
                "Unable to determine download URL: \(ServerError.detailedMessage(from: error))")
        }
