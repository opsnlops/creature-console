...
extension CreatureServerClient {
    ...
    public func connectWebsocket(processor: MessageProcessor) async {
        ...
        // Tear down any existing client first so this is safe to call as a "reconnect".
        // Without this, the old client leaks *live*: its socket and pipeline task keep the
        // actor alive, its own auto-reconnect keeps it online, and both clients feed the
        // processor — every server message gets handled twice (issue #45).
        if let existing = webSocketClient {
    ...
    actor WebSocketClient {
            ...
            private func handlePathUpdate(_ path: NWPath) async {
                if path.status == .satisfied {
                    ...
                    if isConnected || isConnecting || task != nil {
                        ...
                        task?.cancel(
                            with: .goingAway, reason: "Network unavailable".data(using: .utf8))
                        task = nil
                    }
            ...
            Task { [weak self] in
                do {
                    ...
                    if await self?.shouldStayConnected == true,
                        await self?.isNetworkAvailable() == true
                    {
                    ...
                    if await self.isConnected {
                        await self.task?.sendPing { [weak self] error in
                            Task {
                                ...
                                if let error = error {
                                    self.logger.warning(
                                        "Ping failed: \(error.localizedDescription)")
        ...
        func sendMessage(_ message: String) async -> Result<String, ServerError> {
            ...
            guard isConnected else {
                return .failure(
                    .websocketError("Unable to send message because we're not connected"))
            }
