...
### 0.5 What is not finished (rewritten 2026-09-21)

Phases 1-9 are built and running (facts, presence, the house, scenes, the Bridge, tools, search,
beliefs, per-bird memory, retrieval). Left:

- **Phase 10 - replay and operational hardening.** Timeline scrubbing in the Viewer ("turn back
  time"), versioned reducers with golden replays, backup/restore, `.deb` upgrade tests.
- **Loose threads:** `sighting.identified` on arrival (world); a "notice" kind for allowed text
  senders beyond deliveries (Bridge); weather beside departures; `chorus: when_invited` tried
  for a day; Mango's empty turn (no text, no `[pass:]`) on a 2026-09-19 back-door scene; the
  stray `person:mukilteo-clinton-ferry` entity; the pre-existing `HousePresenceTests` flake on
  shared Mongo; Kenny's own voice in memory to be read after the first three-bird night.
- **STT**, last by design; `creature-listener` 0.2.0 needs the Pi 5 + mic test and the wake word.
- Housekeeping: GitHub Support purge after the 2026-09-18 history rewrite; re-signing those 41
  commits if wanted.
