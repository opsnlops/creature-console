...
order lookup by items). Bridge `0.6.3`: carrier windows become days, `order.last_heard`. Bridge `0.7.0`: the person map moved onto the cards (URL labeled Beaky) and event notes can name the person. Bridge `0.8.0`: step 6, Messages via `chat.db`, built. Bridge `0.9.0`: appointments from mail become events and house visitors (the pest control reminder). All six steps are built; what remains is living with them — and the founding moment awaits its first
