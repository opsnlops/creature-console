...
door and the driveway pushed out. Applies to scenes and to the nightly digest.
...
`audience` beside `meaning`; leaving it out leaves it as it was. **Links, one hop:** a fact whose
value is an entity id of a known kind (`person`, `place`, `house`, `character`, `thing`, `event`,
...
other; the glossary answers for the family. Every cast is keyed by the `memory.consolidate` event that asked, so a retry of the same night is idempotent; a day asked for again is remembered afresh and *replaces* the earlier memory of it — the job first takes back every `memory.episode.<day>.*` and `memory.reflection.<day>` the world holds, found with `GET /v1/facts?predicate_prefix=memory.episode.<day>.` (`0.24.0`; the same filter serves any family across every subject), then casts the new ones. Memories have no `valid_to`, so retention never removes them; a mind is handed only the
`episodes_in_prompt` most salient and newest within `episode_days`, and its `reflections_in_prompt`
newest reflections — fetched apart from the day's facts (`0.24.0`), so a night's episodes never
push what April taught the birds yesterday off the 40-fact page. To remember a day by hand — the first night, or a day whose memory you want redone —
