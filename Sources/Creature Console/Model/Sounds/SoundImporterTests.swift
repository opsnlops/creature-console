...
struct SoundImporterTests {
    ...
    func upsertBatchInsertsNew() async throws {
        ...
        let dtos = [
            Common.Sound(fileName: "sound1.wav", size: 1024, transcript: "First sound"),
            Common.Sound(fileName: "sound2.wav", size: 2048, transcript: "Second sound"),
        ]
        ...
        #expect(results.contains { $0.id == "sound1.wav" && $0.transcript == "First sound" })
        #expect(results.contains { $0.id == "sound2.wav" && $0.transcript == "Second sound" })
    }
    ...
    func upsertBatchUpdatesExisting() async throws {
        ...
        let initialDTO = Common.Sound(
            fileName: "sound1.wav", size: 1024, transcript: "Original transcript")
        try await importer.upsertBatch([initialDTO])

        // Update with new data
        let updatedDTO = Common.Sound(
            fileName: "sound1.wav", size: 2048, transcript: "Updated transcript")
    ...
    func deleteAllExceptRemovesOthers() async throws {
        ...
        let dtos = [
            Common.Sound(fileName: "keep1.wav", size: 1024, transcript: "Keep 1"),
            Common.Sound(fileName: "keep2.wav", size: 2048, transcript: "Keep 2"),
            Common.Sound(fileName: "delete.wav", size: 4096, transcript: "Delete Me"),
        ]
    ...
    func upsertBatchMixedData() async throws {
        ...
        // Insert initial data
        let initialDTO = Common.Sound(fileName: "existing.wav", size: 1024, transcript: "Existing")
        try await importer.upsertBatch([initialDTO])

        // Upsert with one existing and one new
        let mixedDTOs = [
            Common.Sound(fileName: "existing.wav", size: 2048, transcript: "Updated Existing"),
            Common.Sound(fileName: "new.wav", size: 4096, transcript: "Brand New"),
        ]
    ...
    func upsertBatchWithEmptyTranscript() async throws {
        ...
        let dto = Common.Sound(fileName: "silent.wav", size: 512, transcript: "")
    ...
    func upsertBatchPreservesExtensions() async throws {
        ...
        let dtos = [
            Common.Sound(fileName: "sound.wav", size: 1024, transcript: "WAV file"),
            Common.Sound(fileName: "sound.mp3", size: 2048, transcript: "MP3 file"),
            Common.Sound(fileName: "sound.m4a", size: 4096, transcript: "M4A file"),
        ]
        try await importer.upsertBatch(dtos)
