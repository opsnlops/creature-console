...
actor AppBootstrapper {
    ...
    public func startIfNeeded() async {
        ...
        let imports = await importCachesSequentially()
    ...
    /// SwiftData imports all share the application's container. Keeping them ordered avoids
    /// concurrent model-cache/context work, which has caused intermittent Core Data crashes.
    private func importCachesSequentially() async -> [(
        label: String, result: Result<String, any Error>
    )] {
        [
            (
                "Creature cache",
                (await CreatureManager.shared.populateCache()).mapError { $0 as Error }
            ),
            ("Animation metadata", await importAnimationsIntoSwiftData()),
            ("Playlists", await importPlaylistsIntoSwiftData()),
            ("Sounds", await importSoundsIntoSwiftData()),
            ...
            ("Music library", await importMusicPiecesIntoSwiftData()),
        ]
    }
    ...
    private func importMusicPiecesIntoSwiftData() async -> Result<String, Error> {
        do {
            let container = await SwiftDataStore.shared.container()
            let importer = MusicPieceImporter(modelContainer: container)
            switch await CreatureServerClient.shared.listMusicPieces() {
            case .success(let list):
                try await importer.upsertBatch(list)
                return .success("Imported \(list.count) music pieces")
            case .failure(let serverError):
                return .failure(serverError)
            }
        } catch {
